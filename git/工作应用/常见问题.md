...
* [git pull时遇到error: cannot lock ref的错误](#git pull时遇到error: cannot lock ref的错误)
...
### git pull时遇到error: cannot lock ref的错误

#### 现象

git pull的时候会遇到错误信息，导致无法更新

```
rror: 'refs/heads/dev/sub' exists; cannot create 'refs/heads/dev/sub/master'
```

#### 原因

参考网址：https://stackoverflow.com/questions/22630404/git-push-refs-heads-my-subbranch-exists-cannot-create

存在refs/heads/dev/sub分支了，所以再创建`refs/heads/dev/sub/master`这种目录结构的分支时会报错，所以建议使用`-`来分割分支名么？不要使用`/`创建文件夹的形式来创建新分支。
